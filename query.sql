...
SELECT M.numero_posti - COUNT(B.id) AS posti_disponibili
FROM Corse C
JOIN Mezzi M ON C.targa_mezzo = M.targa_mezzo
LEFT JOIN Biglietti B ON B.id_corsa = C.id
WHERE C.id = 29;
...
SELECT B.id, C.data, C.ora_partenza,
  CASE 
